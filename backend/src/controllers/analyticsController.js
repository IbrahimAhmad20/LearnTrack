...
async function aggregateActiveStudentsForCourses(
  courseIds,
  { start, end } = {},
) {
  ...
  const contentIdToCourse = Object.fromEntries(
    contents.map((c) => [c.content_id, c.course_id]),
  );
  ...
  let q = supabase
    .from("activity_log")
    .select(
      "user_id, watch_time, content_id, event_at, users!inner(full_name, role)",
    )
...
async function underperformingForCourses(courseIds, threshold) {
  ...
  const quizToCourse = Object.fromEntries(
    quizzes.map((q) => [q.quiz_id, q.course_id]),
  );
  ...
  // [F7] Bucket by (user_id, course_id) — a student can underperform in one
  //      course but not another; the old flat uid key masked that distinction.
  const buckets = {};
  for (const row of attempts || []) {
    ...
    buckets[key].scores.push(Number(row.score));
  }

  return Object.values(buckets)
    .map((b) => ({
      user_id: b.user_id,
      course_id: b.course_id, // [F7] now included per (user, course) grain
...
async function activeStudents(req, res, next) {
  try {
    ...
    if (req.user.role === "instructor") {
      ...
      const rows = await aggregateActiveStudentsForCourses(courseIds, {
        start,
        end,
      });
...
async function underperforming(req, res, next) {
  try {
    ...
    if (req.user.role === "instructor") {
      let courseIds = await getInstructorCourseIds(req.user.user_id);
      if (!courseIds.length) return res.json([]); // guard: instructor has no courses
      if (req.query.course_id) {
        const want = Number(req.query.course_id);
        if (!courseIds.includes(want)) {
          return res
    ...
    const { data, error } = await supabase
      ...
      // [F7] MV now has (user_id, course_id) grain — select course_id too
      .select("user_id, full_name, course_id, avg_score, attempts_count")

...
try { app.use(cookieParser()); } catch (_) {}

app.use('/api/v1/auth',        authRoutes);
app.use('/api/v1/users',       userRoutes);
app.use('/api/v1/courses',     courseRoutes);
app.use('/api/v1/enrollments', enrollmentRoutes);   // v2: new dedicated route
...
if (require.main === module) {
  (async () => {
    await connectDB();
    app.listen(PORT, () => {
      console.log(`LearnTrack API running on port ${PORT} [${process.env.NODE_ENV}]`);
    });
  })();
} else {
